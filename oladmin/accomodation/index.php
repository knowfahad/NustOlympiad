...
if($formsubmitted){
    ...
    if(strlen($challanid)){
        $stmt = $mpdo->prepare("select * from challan as c inner join participant as p on p.AccomodationChallanID = c.ChallanID where c.ChallanID = ?");
        $stmt->execute([$challanid]);
        if(!$stmt->rowCount()){
            $errors[] = "Wrong Accomodation Challan ID.";
        }
        else{
