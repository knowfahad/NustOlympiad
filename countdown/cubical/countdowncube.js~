    ...
    var helpers = { 
            ...
            ***********************************************************/
            ...
            this.classes = [ 'front','back', 'right', 'left', 'top', 'bottom' ];	
                    ...
                    for( var tagIndex in this.topTags )
                    {
                            ...
                            if(tag == 'year' || tag =='month')
                                continue;       
                            cube = element.append('<section style="margin-left:5%;"></section>')
                                       .children(':last')	   
                                            .attr('id', tag)
    		...
    		***********************************************************/
        	shiftCube: function( element, options, cube, nextNumber )
        	{
        		...
        		if( nextNumber != lastNumber )
        		{
                    ...
                    if(lastClass && lastClass.replace){
    		...
    		***********************************************************/
    		...
    		setTimeLeft: function( element, options )
    		{
                ...
                /*time difference edited by mukarram*/
                
                var daysToShow, monthsToShow, hoursToShow, minutesToShow, secondsToShow;
    	    	// get total seconds between the times
                var delta = Math.abs(options.target - now) / 1000;

                // calculate (and subtract) whole days
                var myDays = Math.floor(delta / 86400);
                delta -= days * 86400;

                // calculate (and subtract) whole hours
                var myHours = Math.floor(delta / 3600) % 24;
                delta -= hours * 3600;

                // calculate (and subtract) whole minutes
                var myMinutes = Math.floor(delta / 60) % 60;
                delta -= minutes * 60;
                // what's left is seconds
                var mySeconds = delta % 60;
    	    	...
    	    	if( days < 0 || ( days == 0 && hours < 0 ) || ( days == 0 && hours == 0 && minutes < 0 ) || ( days == 0 && hours == 0 && minutes == 0 && seconds <= 0 ) )
    	    	{
    	    		...
    	    		daysToShow = Math.floor((options.target-now)/(60*60*24)/1000);
                    monthsToShow --;
    	    	}
    	    	else if( days == 0 )
    	    	{
    	    	...
    	    	diffTime = new Date((options.target - now)-(5*60*60*1000));
    	    	/*daysToShow = myDays;
                secondsToShow = mySeconds;
                minutesToShow = myMinutes;
                hoursToShow   = myHours;*/
